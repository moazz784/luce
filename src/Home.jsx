 ...
 useEffect(() => {
  ...
  return (
    ...
    <div id='100' className="min-h-screen font-sans selection:bg-green-500 selection:text-white overflow-x-hidden">
      ...
      {/* --- 1. Navbar --- */}
      <nav className="bg-[#1a2b56] text-white px-4 md:px-8 py-3 flex items-center justify-between sticky top-0 z-[100] shadow-xl font-sans h-[80px]">
      
      {/* 1. Logo Section (يسار) */}
      <div className="flex items-center gap-3 h-full">
        <div className="w-25 h-25 rounded-full flex items-center justify-center p-1 ">
      ...
      <div className="flex items-center gap-4 border-l border-white/20 pl-5 h-full py-5">
        ...
        <button className="text-white hover:text-green-400 transition-all p-1 border border-white/10 rounded hover:border-green-400/50">
          <Menu onClick={() => navigate("/AdminDashboard")} size={24} className="cursor-pointer" />
        </button>
        <button onClick={() => navigate("/login")} className='rounded-3xl bg-green-500 font-bold cursor-pointer hover:bg-cyan-500 py-1.5 px-1.5'>
            login
        </button>
      </div>
      ...
      <section className="relative h-[480px] md:h-[550px] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
          <div className="text-center px-4 max-w-4xl pointer-events-auto">
             <h2 className="text-5xl md:text-5xl font-extrabold mb-6 text-white drop-shadow-2xl">Alumni Society</h2>
            ...
            {alumni.map((person) => (
              <SwiperSlide key={person.id} className="flex justify-center">
                <div className="group bg-white rounded-2xl p-6 text-center h-[420px] w-full max-w-[320px] flex flex-col items-center shadow-xl relative overflow-hidden transition-all hover:-translate-y-2">
                  <div className="w-30 h-30 rounded-full border-4 border-gray-100 overflow-hidden mb-2 group-hover:border-green-400 transition-colors">
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in zoom-in duration-300">
              ...
              <div className="md:w-1/3 bg-gray-50 p-8 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-gray-100">
                <img src={selectedAlumnus.image} className="w-33 h-33 rounded-full border-4 border-green-400 mb-4 shadow-md" alt="" />
...
<section id="awards" className="py-24 bg-white overflow-hidden">
  <div className="container mx-auto px-6 lg:px-20">
    ...
    <div className="relative bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-gray-100 min-h-[550px] flex flex-col lg:flex-row">
      
      <AnimatePresence mode="wait">
        {/* Content Side */}
        <motion.div
          key={`text-${awardIndex}`} // ✅ تم التغيير لـ awardIndex
          initial={{ opacity: 0, x: -50 }}
          animate={{ opacity: 1, x: 0 }}
          exit={{ opacity: 0, x: 50 }}
          transition={{ duration: 0.5 }}
          className="flex-1 p-8 lg:p-16 flex flex-col justify-center bg-white z-10"
        >
        ...
        {/* Image Side */}
        <motion.div
          key={`img-${awardIndex}`} // ✅ تم التغيير لـ awardIndex
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-l from-black/5 to-transparent hidden lg:block"></div>
        </motion.div>
      </AnimatePresence>

      {/* Pagination Dots (نقط التنقل) */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 z-30">
        {awards.map((_, i) => (
          <button
            key={i}
            onClick={() => setAwardIndex(i)} // ✅ تم التغيير لـ setAwardIndex
            className={`h-2.5 rounded-full transition-all duration-300 ${
              awardIndex === i ? "w-10 bg-[#00a651]" : "w-2.5 bg-gray-300" 
            }`} // ✅ تم التغيير لـ awardIndex
          />
        ))}
      </div>
    </div>
...
<section id="1000" className="py-12 px-4 bg-white font-sans" dir="ltr">
    ...
    {events.map((event) => (
      <div key={event.id} className="flex flex-col group cursor-pointer">
        ...
        <div className="mt-4 space-y-2 px-1">
          {/* Location and Time (المكان والوقت) */}
          <div className="flex flex-wrap items-center text-[11px] text-gray-500 gap-3">
            <span className="flex items-center gap-1">
              <span className="text-[#8ec63f] text-sm"><MapPin size={14} className="text-[#8ec63f]" /></span> 
              <span className="hover:text-[#00a651]">{event.location}</span>
            </span>
            <span className="flex items-center gap-1">
          ...
          {/* Title (العنوان) */}
          <h3 className="font-bold text-[#1a3668] text-[15px] leading-tight hover:underline min-h-[40px]">
            {event.title}
          </h3>
  ...
  <div className="text-center mt-12">
    <button className="bg-[#00a651] hover:bg-[#008d44] text-white font-bold py-3 px-10 rounded-full transition-colors shadow-md">
      See All Events
    </button>
  </div>
</section>
            ...
            <SwiperSlide key={news.id}>
              <div className="flex flex-col group/card cursor-pointer bg-white rounded-md overflow-hidden h-full border border-transparent hover:border-gray-100 transition-all">
                ...
                <div className="py-5 px-1 space-y-3">
                  ...
                  <h3 className="text-sm text-gray-500 line-clamp-4 ">
                    {news.title}
                  </h3>
                </div>
              </div>
...
<section id="3000" className="relative w-full h-[500px] lg:h-[600px]  overflow-hidden">
    ...
    <footer className="bg-[#1a2b56] text-white pt-20 pb-10 border-t-4 border-white/10">
     ...
     <div className="border-t flex justify-center items-center  border-white/10  py-7 text-center text-black text-[15px] opacity-40">
